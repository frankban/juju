...
func (w *unixConfigure) setUpGUI() (func(), error) {
	...
	if u.Scheme == "file" {
		...
		command := curlCommand + " -o $gui/gui.tar.bz2 --retry 10"
		if w.icfg.DisableSSLHostnameVerification {
			command += " --insecure"
		}
		command += " " + shquote(u.String())
		w.conf.AddRunCmd(cloudinit.LogProgressCmd("Fetching GUI archive: %s", command))
	...
	return func() {
		...
		w.conf.AddRunCmd("rm $gui/gui.tar.bz2 $gui/jujugui.sha256 $gui/downloaded-gui.txt")
	}, nil
